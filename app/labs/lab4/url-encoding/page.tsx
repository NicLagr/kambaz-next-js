...
export default function UrlEncoding() {
  ...
  return (
    <div style={{ padding: 40, maxWidth: 600 }} id="wd-url-encoding">
      ...
      <Link href={`${baseUrl}/query-params?a=${encodeURIComponent(a)}&b=${encodeURIComponent(b)}`} className="btn btn-primary w-100">
        {a} + {b} → Query Params (Link)
      </Link>
      <Link href={`${baseUrl}/path-params/${encodeURIComponent(a)}/${encodeURIComponent(b)}`} className="btn btn-primary w-100 mt-2">
