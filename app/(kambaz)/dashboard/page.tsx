...
export default function Dashboard() {
  ...
  const visibleCourses = isFaculty || showAllEnrollments
    ? courses
    : courses.filter((c: any) =>
        enrollments.some((e: any) => e.user === currentUserId && e.course === c._id)
      );

  const isEnrolled = (courseId: string) =>
    enrollments.some((e: any) => e.user === currentUserId && e.course === courseId);
  ...
  return (
    <div id="wd-dashboard" className="p-4">
      ...
      {!isFaculty && currentUser && (
        <Button className="btn btn-primary float-end mb-2" onClick={() => setShowAllEnrollments(!showAllEnrollments)}>
          Enrollments
        </Button>
      )}
            ...
            <Col key={c._id} className="wd-dashboard-course" style={{ width: "300px" }}>
              <Card>
                <Link href={`/courses/${c._id}/home`} className="wd-dashboard-course-link text-decoration-none text-dark">
                  <CardImg variant="top" src={c.image || "/images/reactjs.jpg"} width="100%" height={160} alt={c.name} />
                  <CardBody>
